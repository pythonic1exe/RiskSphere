...
export function DashboardHeader({ organizationName, userEmail }: { organizationName: string; userEmail?: string }) {
  ...
  return (
    <header className="flex flex-col gap-7 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 className="mt-2 font-heading text-4xl font-medium tracking-[-0.065em] text-text-primary sm:text-[2.85rem]">Dashboard</h1>
        <p className="mt-3 text-sm text-text-muted">Your organization&apos;s risk, compliance and assurance overview across {organizationName}.</p>
      </div>
